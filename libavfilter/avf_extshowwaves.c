...
typedef struct ExtShowWavesContext {
    const AVClass *class;
    int w, h;
    AVRational rate;
    char *colors;
    ...
    int mode;                   ///< ExtShowWavesMode
    int scale;                  ///< ExtShowWavesScale
    int draw_mode;              ///< ExtShowWavesDrawMode
    int split_channels;
    int line_width;
    int space;
    int offset;
    ...
    void (*draw_sample)(uint8_t *buf, int height, int linesize,
                        int16_t *prev_y, const uint8_t color[4], int h, int offset);
...
#define OFFSET(x) offsetof(ExtShowWavesContext, x)
#define FLAGS AV_OPT_FLAG_FILTERING_PARAM|AV_OPT_FLAG_VIDEO_PARAM

static const AVOption extshowwaves_options[] = {
    ...
    { "offset",    "set wave offset from center", OFFSET(offset), AV_OPT_TYPE_INT, {.i64 = 0}, INT_MIN, INT_MAX, FLAGS },
...
AVFILTER_DEFINE_CLASS(extshowwaves);
...
static void draw_sample_point_rgba_scale(uint8_t *buf, int height, int linesize,
                                         int16_t *prev_y,
                                         const uint8_t color[4], int h, int offset)
{
  h+=offset;
    if (h >= 0 && h < height) {
...
{
    int k;
    int start   = height/2 + offset;
    int end     = av_clip(h + offset, 0, height-1);
    if (start > end)
...
inline static int push_frame(AVFilterLink *outlink)
{
    AVFilterContext *ctx = outlink->src;
    AVFilterLink *inlink = ctx->inputs[0];
    ExtShowWavesContext *showwaves = outlink->src->priv;
    int nb_channels = inlink->channels;
    int ret, i;
...
static int push_single_pic(AVFilterLink *outlink)
{
    ...
    const int offset = showwaves->offset;
    int col = 0;
        ...
        for (i = 0; i < frame->nb_samples; i++) {
            ...
            if (n == max_samples) {
                for (ch = 0; ch < nb_channels; ch++) {
                    ...
                    showwaves->draw_sample(buf, ch_height, linesize, &showwaves->buf_idy[ch], &showwaves->fg[ch * 4], h, offset);
                    sum[ch] = 0;
                }
                col++;
                n = 0;
...
static int alloc_out_frame(ExtShowWavesContext *showwaves, const int16_t *p,
...
static int showwaves_filter_frame(AVFilterLink *inlink, AVFrame *insamples)
{
    ...
    for (i = 0; i < nb_samples; i++) {
        ...
        for (j = 0; j < nb_channels; j++) {
            ...
            for (size_t i = 0; i <= showwaves->line_width; i++) {
              /* code */
              showwaves->draw_sample(buf, ch_height, linesize,
                                     &showwaves->buf_idy[j], &showwaves->fg[j * 4], h, offset);
              buf += pixstep;
            }
...
AVFilter ff_avf_extshowwaves = {
    .name          = "extshowwaves",
    .description   = NULL_IF_CONFIG_SMALL("Convert input audio to a video output."),
    .init          = init,
    .uninit        = uninit,
    .query_formats = query_formats,
    .priv_size     = sizeof(ExtShowWavesContext),
    .inputs        = showwaves_inputs,
    .outputs       = showwaves_outputs,
    .priv_class    = &extshowwaves_class,
};

#endif // CONFIG_EXTSHOWWAVES_FILTER

#if CONFIG_EXTSHOWWAVESPIC_FILTER

#define OFFSET(x) offsetof(ExtShowWavesContext, x)
#define FLAGS AV_OPT_FLAG_FILTERING_PARAM|AV_OPT_FLAG_VIDEO_PARAM

static const AVOption extshowwavespic_options[] = {
    { "size", "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS },
    { "s",    "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS },
    { "split_channels", "draw channels separately", OFFSET(split_channels), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS },
    { "colors", "set channels colors", OFFSET(colors), AV_OPT_TYPE_STRING, {.str = "red|green|blue|yellow|orange|lime|pink|magenta|brown" }, 0, 0, FLAGS },
...
AVFILTER_DEFINE_CLASS(extshowwavespic);
...
AVFilter ff_avf_extshowwavespic = {
    ...
    .priv_class    = &extshowwavespic_class,
};

#endif // CONFIG_EXTSHOWWAVESPIC_FILTER
