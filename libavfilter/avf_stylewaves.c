...
extern void draw_line(buffer_conext,
...
typedef struct StyleWavesContext
{
    const AVClass *class;
    int w, h;
    AVRational rate;
    ...
    int mode;      ///< StyleWavesMode
    int scale;     ///< StyleWavesScale
    int draw_mode; ///< StyleWavesDrawMode
    int split_channels;
    int line_width;
    int space;
    int fill_shape; ///< StyleWavesShape
    uint8_t *fg;

    int (*get_h)(int16_t sample, int height);
    void (*draw_sample)(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample, int h, enum StyleWavesShape shape);
...
#define OFFSET(x) offsetof(StyleWavesContext, x)
#define FLAGS AV_OPT_FLAG_FILTERING_PARAM | AV_OPT_FLAG_VIDEO_PARAM

static const AVOption stylewaves_options[] = {
    {"size", "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS},
    {"s", "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS},
    {"mode", "select display mode", OFFSET(mode), AV_OPT_TYPE_INT, {.i64 = MODE_HORIZONTAL}, 0, MODE_NB - 1, FLAGS, "mode"},
    {"horizontal", "draw horizontal", 0, AV_OPT_TYPE_CONST, {.i64 = MODE_HORIZONTAL}, .flags = FLAGS, .unit = "mode"},
    {"centered", "draw centered", 0, AV_OPT_TYPE_CONST, {.i64 = MODE_CENTERED}, .flags = FLAGS, .unit = "mode"},
    ...
    {"linewidth", "set line width", OFFSET(line_width), AV_OPT_TYPE_INT, {.i64 = 1}, 0, INT_MAX, FLAGS},
    {"lw", "set line width", OFFSET(line_width), AV_OPT_TYPE_INT, {.i64 = 1}, 0, INT_MAX, FLAGS},
    {"space", "set line space", OFFSET(space), AV_OPT_TYPE_INT, {.i64 = 0}, 0, INT_MAX, FLAGS},
    {"fill", "set filled shape", OFFSET(fill_shape), AV_OPT_TYPE_INT, {.i64 = SHAPE_DOT}, INT_MIN, INT_MAX, FLAGS},
...
AVFILTER_DEFINE_CLASS(stylewaves);

static av_cold void uninit(AVFilterContext *ctx)
{
    StyleWavesContext *showwaves = ctx->priv;
...
static void draw_sample_dot_scale(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
    draw_line(buf, 0, 0, h, h, color);
}

static void draw_sample_dot_full(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_cd_scale(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_cd_full(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_square_scale(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_square_full(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_wave_scale(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}
static void draw_sample_wave_full(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_waveline_scale(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}

static void draw_sample_waveline_full(const buffer_conext buf, const uint8_t color[4], int sample, int total_sample,int h, enum StyleWavesShape shape)
{
}
...
static int config_output(AVFilterLink *outlink)
{
    ...
    switch (showwaves->fill_shape)
    {
    case SHAPE_DOT:
        switch (showwaves->draw_mode)
        {
        case DRAW_SCALE:
            showwaves->draw_sample = draw_sample_dot_scale;
        case DRAW_FULL:
            showwaves->draw_sample = draw_sample_dot_scale;
        default:
            return AVERROR_BUG;
    ...
    case SHAPE_CD:
        ...
        {
        case DRAW_SCALE:
            showwaves->draw_sample = draw_sample_cd_scale;
        case DRAW_FULL:
            showwaves->draw_sample = draw_sample_cd_scale;
        default:
            return AVERROR_BUG;
    ...
    case SHAPE_SQUARE:
        ...
        {
        case DRAW_SCALE:
            showwaves->draw_sample = draw_sample_square_scale;
        case DRAW_FULL:
            showwaves->draw_sample = draw_sample_square_scale;
        default:
            return AVERROR_BUG;
    ...
    case SHAPE_WAVE:
        ...
        {
        case DRAW_SCALE:
            showwaves->draw_sample = draw_sample_wave_scale;
        case DRAW_FULL:
            showwaves->draw_sample = draw_sample_wave_scale;
        default:
            return AVERROR_BUG;
    ...
    case SHAPE_WAVELINE:
        ...
        {
        case DRAW_SCALE:
            showwaves->draw_sample = draw_sample_waveline_scale;
        case DRAW_FULL:
            showwaves->draw_sample = draw_sample_waveline_scale;
        default:
            return AVERROR_BUG;
...
inline static int push_frame(AVFilterLink *outlink)
{
    AVFilterContext *ctx = outlink->src;
    AVFilterLink *inlink = ctx->inputs[0];
    StyleWavesContext *showwaves = outlink->src->priv;
    int nb_channels = inlink->channels;
    int ret, i;
...
static int push_single_pic(AVFilterLink *outlink)
{
    ...
    const int fill_shape = showwaves->fill_shape;
    int col = 0;
    ...
    for (node = showwaves->audio_frames; node; node = node->next)
    {
        ...
        buffer_conext buf_context;
        buf_context.pixels = frame->data[0];
        buf_context.width = outlink->w;
        buf_context.height = outlink->h;
        buf_context.pixstep = pixstep;
        buf_context.linesize = linesize;
        ...
        for (i = 0; i < frame->nb_samples; i++)
        {
            ...
            if (n == max_samples)
            {
                for (ch = 0; ch < nb_channels; ch++)
                {
                    ...
                    showwaves->draw_sample(buf_context, &showwaves->fg[ch * 4], i, frame->nb_samples, h, fill_shape);
                    sum[ch] = 0;
                }
                col++;
                n = 0;
...
static int alloc_out_frame(StyleWavesContext *showwaves, const int16_t *p,
...
static av_cold int init(AVFilterContext *ctx)
{
    ...
    if (!strcmp(ctx->filter->name, "styledwavespic"))
    {
        showwaves->single_pic = 1;
        showwaves->mode = MODE_HORIZONTAL;
    }

    return 0;
}

#if CONFIG_STYLEWAVES_FILTER

static int styledwaves_filter_frame(AVFilterLink *inlink, AVFrame *insamples)
{
    ...
    for (i = 0; i < nb_samples; i++)
    {
        ...
        buffer_conext buf_context;
        buf_context.pixels = outpicref->data[0];
        buf_context.width = outlink->w;
        buf_context.height = outlink->h;
        buf_context.pixstep = pixstep;
        ...
        {
            int h = showwaves->get_h(*p++, ch_height);
            showwaves->draw_sample(buf_context, &showwaves->fg[j * 4], i, nb_samples, h, fill_shape);
        }
        ...
        if (showwaves->sample_count_mod == n)
        {
            ...
            showwaves->buf_idx += showwaves->space + showwaves->line_width;
        }
...
static const AVFilterPad styledwaves_inputs[] = {
    {
        .name = "default",
        .type = AVMEDIA_TYPE_AUDIO,
        .filter_frame = styledwaves_filter_frame,
    },
    {NULL}};

static const AVFilterPad styledwaves_outputs[] = {
    {
        .name = "default",
...
AVFilter ff_avf_stylewaves = {
    .name = "stylewaves",
    .description = NULL_IF_CONFIG_SMALL("Convert input audio to a video output."),
    .init = init,
    .uninit = uninit,
    .query_formats = query_formats,
    .priv_size = sizeof(StyleWavesContext),
    .inputs = styledwaves_inputs,
    .outputs = styledwaves_outputs,
    .priv_class = &stylewaves_class,
};

#endif // CONFIG_STYLEWAVES_FILTER

#if CONFIG_STYLEWAVESPIC_FILTER

#define OFFSET(x) offsetof(StyleWavesContext, x)
#define FLAGS AV_OPT_FLAG_FILTERING_PARAM | AV_OPT_FLAG_VIDEO_PARAM

static const AVOption stylewavespic_options[] = {
    {"size", "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS},
    {"s", "set video size", OFFSET(w), AV_OPT_TYPE_IMAGE_SIZE, {.str = "600x240"}, 0, 0, FLAGS},
    {"split_channels", "draw channels separately", OFFSET(split_channels), AV_OPT_TYPE_BOOL, {.i64 = 0}, 0, 1, FLAGS},
    {"colors", "set channels colors", OFFSET(colors), AV_OPT_TYPE_STRING, {.str = "red|green|blue|yellow|orange|lime|pink|magenta|brown"}, 0, 0, FLAGS},
...
AVFILTER_DEFINE_CLASS(stylewavespic);
...
AVFilter ff_avf_stylewavespic = {
    ...
    .priv_class = &stylewavespic_class,
};

#endif // CONFIG_STYLEWAVESPIC_FILTER
